        ...
        <div className="container mx-auto">
          <div className="relative max-w-xl mx-auto">
            <input
              ...
              className="w-full pl-12 pr-4 py-3 border rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-primary"
              value={searchTerm}
        ...
        <div className="container mx-auto">
          {filteredGroups.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {filteredGroups.map(group => (
                <Link key={group.id} href={`/groups/${group.id}`} className="block">
                  <div className={`card ${group.id === 'megadeth' || group.id === 'blacksabbath' ? 'low-fi' : 'high-fi text-white'} h-full`}>
                    ...
                    <p className={`${group.id === 'megadeth' || group.id === 'blacksabbath' ? 'text-gray-700' : ''} mb-4`}>
                      {group.description}
                    </p>
            ...
            <div className="text-center py-12">
              <h2 className="text-2xl font-bold text-gray-500 mb-2">Ничего не найдено</h2>
